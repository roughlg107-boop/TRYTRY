...
| 情境 | 處理方式 |
|------|----------|
| STEP 1 某欄位為空字串或標示於 `missing_information` | 該欄位在填空時以「**這一段我們等下確認**」帶過；**仍須生成該頁**，不得跳過或合併。 |
| 禁止 AI 補齊或推測 | 不得依「可能」「推測」「一般來說」等補齊缺漏；僅能填入 STEP 1 中**明確存在**的內容，或「這一段我們等下確認」。 |
| 禁止使用「待確認」 | 缺漏時**不得**用「待確認」當作分析結果；僅允許「這一段我們等下確認」。 |

- 若整頁所需變數皆缺漏，該頁標題與內文仍依 `title_pattern` 與句型產出，缺漏處一律「這一段我們等下確認」。  
- **禁止**因資料不足而刪除頁面、改寫句型為「本頁暫無資料」等說明性文字（僅允許在佔位處寫「這一段我們等下確認」）。  
- 詳見 **`docs/簡報生成-強制修正指令.md`**（角色、語言風格、禁止詞彙、標題與內容規則）。
